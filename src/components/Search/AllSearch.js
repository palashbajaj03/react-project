...
import { routingConstants, timezoneDatePrint } from '../../constants';
...
class AllSearch extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.repChannelId !== this.props.repChannelId) {
      if (((cta !== undefined && Object.keys(cta).length !== 0 && cta) || val !== '') && this.state.resetToggle) {
  ...
  componentDidMount() {
    if (this.props.starred !== undefined) {
    ...
    if ((cta !== undefined && Object.keys(cta).length !== 0 && cta) || val !== '') {
      ...
      cta.filter.signals !== undefined && cta.filter.signals.opportunity !== undefined && temp_opportunity.push({ type: "opportunity", opportunity: (cta.filter.signals.opportunity.value === '' ? 'Any+' : cta.filter.signals.opportunity.value === 1 ? 'Yes' : cta.filter.signals.opportunity.value === 0 ? 'No' : ''), string: "Opportunity: " + (cta.filter.signals.opportunity.value === '' ? 'Any' : cta.filter.signals.opportunity.value === 1 ? 'Yes' : cta.filter.signals.opportunity.value === 0 ? 'No' : ''), oppVal: cta.filter.signals.opportunity.value });
      let temp_journey = []
  ...
  saveButtonClicked = () => {
    ...
    this.props.save({
      data, search_strings, temp_search_channel, currentselectedoption, temp_finaldate_range,
      temp_searchspeaker,
      temp_finaldate,
      temp_opportunity,
      temp_journey,
  ...
  checkDate = () => {
    if (this.state.filter.date.from) {
      return true
    } else {
      this.setState(prevState => ({
        filter: {
          ...prevState.filter,
          search_conversation_date_from: this.props.searchFilter.date.min,
          search_conversation_date_to: this.props.searchFilter.date.max,
        }
      }))
      return true;
    }
  ...
  handleSaveToCollection = async () => {
    ...
    const { collectionName, end_date } = this.state;
    if (collectionName) {
  ...
  handleCheckBox = (e) => {
    if (e.target.id === 'engagement' && e.target.checked) {
      this.setState((prevState) => {
        return {
          engagement: true,
    ...
    } else if (e.target.id === 'signals' && e.target.checked) {
      this.setState((prevState) => ({
        signals: true,
        counter: prevState.counter + 1
      }))
    } else if (e.target.id === 'topics' && e.target.checked) {
      this.setState((prevState) => ({
        topic: true,
        counter: prevState.counter + 1
      }))
  ...
  replaceKeys = (inputString, keys) => {
    ...
    let FF = s.replace(new RegExp(placeholder, 'g'), 'strong>');
    return { __html: FF }
  }
  render() {
          ...
          <div className="custom-row">
            <div className="start-all-search">
              <div className="input-hide-search">{
                this.state.inputsearch && <React.Fragment><input type="text" onChange={this.onChangeTextSearch} onKeyUp={this.onChangeTextSearch} id="search-scoop" placeholder="Search with some keywords" autoFocus maxLength="512" />
                                            ...
                                            conversation.preview.reps.map((data, index) => {
                                              return <React.Fragment key={index}>
                                                <span className="msg">
                                                  ...
                                                  {
                                                    <div dangerouslySetInnerHTML={this.replaceKeys(data.message, this.state.search_strings)}></div>
                                                  }
                                                </span>
                                              </React.Fragment>
                                            })
...
const mapStateToProps = state => {
  return {
    ...
    recentlySavedCollection: state.searchReducer.recentlySavedCollection,
    starred: state.collectionReducer.starredConversation,
    collectionList: state.conversationReducer.manualCollection,
    filterState: state.searchReducer.filterState
  }
}
...
const mapActionToProps = {
  ...
  loadSavedCollection: searchAction.loadRecentlyUpdatedList,
...
export default withRouter(connect(mapStateToProps, mapActionToProps)(AllSearch));